...
nvinfer1::IIdentityLayer* convert_to_int(nvinfer1::ILayer* op, ConversionCtx* ctx, const torch::jit::Node* n) {
    ...
    RegisterNodeConversionPatterns()
        ...
        .pattern({"aten::gt.Tensor(Tensor self, Tensor other) -> (Tensor)",
                  [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                    ...
                    auto identity = convert_to_int(gt, ctx, n);
        ...
        .pattern({"aten::lt.Tensor(Tensor self, Tensor other) -> (Tensor)",
                  [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                    ...
                    auto identity = convert_to_int(lt, ctx, n);
                    auto out = ctx->AssociateValueAndTensor(n->outputs()[0], identity->getOutput(0));
                    LOG_DEBUG("Output tensor shape: " << out->getDimensions());
                    return true;
                  }})
        ...
        .pattern({"aten::eq.Tensor(Tensor self, Tensor other) -> (Tensor)",
                  [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                    ...
                    auto identity = convert_to_int(eq, ctx, n);
                    auto out = ctx->AssociateValueAndTensor(n->outputs()[0], identity->getOutput(0));
                    LOG_DEBUG("Output tensor shape: " << out->getDimensions());
                    return true;
                  }})
        ...
        .pattern({"aten::ge.Tensor(Tensor self, Tensor other) -> (Tensor)",
                  [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                    ...
                    auto identity = convert_to_int(or_op, ctx, n);
                    auto out = ctx->AssociateValueAndTensor(n->outputs()[0], identity->getOutput(0));
                    LOG_DEBUG("Output tensor shape: " << out->getDimensions());
                    return true;
                  }})
